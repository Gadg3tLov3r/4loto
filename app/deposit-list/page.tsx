...
import {
  getDepositLogs,
  type DepositLogsResponse,
  type DepositLogsFilters,
} from "@/lib/api-client";
import {
  Card,
  CardContent,
...
export default function DepositListPage() {
  ...
  const [data, setData] = useState<DepositLogsResponse | null>(null);
  ...
  const fetchDepositLogs = async () => {
    try {
      ...
      const filters: DepositLogsFilters = {
        page,
        page_size: pageSize,
      };
      ...
      const response = await getDepositLogs(filters);
      setData(response);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load deposit logs");
    } finally {
      setLoading(false);
    }
  };
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-8">
              ...
              <Button onClick={fetchDepositLogs}>Retry</Button>
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>View and filter your deposit transactions</CardDescription>
        </CardHeader>
        <CardContent>
          {/* Filters */}
                          ...
                          <TableRow key={item.order_id}>
                            ...
                            <TableCell>
                              {item.received_amount !== null
                                ? formatCurrency(item.received_amount, item.currency)
                                : "-"}
                            </TableCell>
